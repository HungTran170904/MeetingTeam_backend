...
public class TokenServerAssistant {
    ...
    static public class TokenInfo {
         ...
         * Error message.
         */
        public TokenServerAssistant.ErrorInfo error;

        TokenInfo() { this.error = new TokenServerAssistant.ErrorInfo(); }
    ...
    static public TokenInfo generateToken04(long appId, String userId, String secret,
        ...
        if (userId == null || userId == "" || userId.length() > 64) {
            token.error.code = TokenServerAssistant.ErrorCode.ILLEGAL_USER_ID;
        ...
        if (secret == null || secret == "" || secret.length() != 32) {
            token.error.code = TokenServerAssistant.ErrorCode.ILLEGAL_SECRET;
        ...
        if (effectiveTimeInSeconds <= 0) {
            token.error.code = TokenServerAssistant.ErrorCode.ILLEGAL_EFFECTIVE_TIME;
...
//         ThreadLocalRandom.current().nextBytes(ivBytes);
        ...
        json.addProperty("ctime", nowTime);
        json.addProperty("expire", expire_time);
        ...
        try {
            ...
            token.error.code = TokenServerAssistant.ErrorCode.SUCCESS;
        } catch (Exception e) {
            debugInfo("generate token failed: " + e);
            token.error.code = TokenServerAssistant.ErrorCode.OTHER;
